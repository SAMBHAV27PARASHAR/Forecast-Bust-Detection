    ...
    def get_forecast_stability(
        ...
        if scenario_id == "live_gefs" or not scenario_id:
            ...
            if not prev_data or not curr_data:
                init_str = curr_data.get("meta", {}).get("init_time_utc", "Current") if curr_data else "Current"
                return {
                    ...
                    "message": f"Single operational cycle active ({init_str}). Preceding operational cycle data is pending next sync.",
                    "stability_status": "INSUFFICIENT_DATA",
                    "stability_description": "Single operational reference run active; no preceding initialization cycle in local cache.",
                    "previous_run": None,
                    "current_run": None,
    ...
    def get_what_changed(
        ...
        if not stability.get("has_previous_run"):
            return {
                ...
                "has_previous_run": False,
                "message": stability.get("message") or "Insufficient comparison data: Single operational reference run active; no preceding initialization cycle available in selected scenario.",
                "summary_statement": "INSUFFICIENT_DATA: No preceding initialization cycle available for comparison.",
                "previous_bust_probability": None,
                "current_bust_probability": None,
